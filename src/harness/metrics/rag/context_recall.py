    ...
    def evaluate(
        ...
        if not context_docs:
            return MetricResult(
                ...
                score=0.0,
                passed=False,
                explanation="No context documents provided for context recall evaluation",
                threshold=self.threshold,
            )

        if self._wrapped is None:
            try:
                self._wrapped = _ContextualRecallMetric(threshold=self.threshold)
        ...
        try:
            ...
            return MetricResult(
                ...
                explanation=reason or f"Context recall score: {score:.3f}",
                threshold=self.threshold,
            )
        except Exception as e:
            raise MetricError(f"ContextRecall evaluation failed: {e}") from e
